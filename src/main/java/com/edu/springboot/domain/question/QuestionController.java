...
public class QuestionController {
    ...
    public ResponseEntity<?> writeQuestion(
        ...
        return success 
            ...
            : ResponseEntity.status(500).body(Map.of("success", false, "message", "등록 중 오류가 발생했습니다."));
    }
    ...
    public ResponseEntity<?> updateQuestion(
        ...
        if (success) {
            ...
            return ResponseEntity.status(400).body(Map.of("success", false, "message", "수정이 불가능한 상태이거나 존재하지 않는 질문입니다."));
        }
    }
    ...
    public ResponseEntity<?> deleteQuestion(@PathVariable("questionId") Long questionId) {
        ...
        String msg = switch (result) {
            case "HAS_ANSWERS" -> "이미 답변이 달린 질문은 삭제할 수 없습니다.";
            case "NOT_FOUND" -> "존재하지 않는 질문입니다.";
            default -> "삭제 실패";
        };
    ...
    /* 💡 6. 답변 채택 기능은 AnswerController로 이동되었으므로 여기서 삭제되었습니다. */
}