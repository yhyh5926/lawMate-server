...
public class WebConfig implements WebMvcConfigurer {
	...
	public void addResourceHandlers(ResourceHandlerRegistry registry) {
		...
		// 채팅 관련 파일 경로 매핑
		registry.addResourceHandler("/uploads/chat/**").addResourceLocations("file:uploads/chat/");

		// 변호사 관련 파일(자격증, 프로필 등) 경로 매핑 추가
		registry.addResourceHandler("/uploads/lawyer/**").addResourceLocations("file:uploads/lawyer/");

		// 회원가입 증빙서류 경로 매핑 추가
		registry.addResourceHandler("/uploads/member/**").addResourceLocations("file:uploads/member/");
	}
}